...
async function syncContext(request: Request) {
  ...
  if (supplied && process.env.CARD_CATALOG_CRON_SECRET && supplied === process.env.CARD_CATALOG_CRON_SECRET) return { supabase: createServiceRoleClient(), triggerType: "cron" as const };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (runError) return NextResponse.json({ data: null, error: { code: "SYNC_LOCK_FAILED", message: "无法创建同步任务" } }, { status: 409 });
