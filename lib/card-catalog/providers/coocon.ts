...
export class CooconProvider implements KoreanCardCatalogProvider {
  ...
  getMetadata(): ProviderMetadata {
    if (!this.configured) return { providerId: this.providerId, status: "not_configured", displayName: "쿠콘 / Coocon", coverage: "需签约后以正式合同为准", containsCompleteBenefits: false, message: "尚未配置 Coocon 商业信用卡数据供应商" };
    if (!this.client) return { providerId: this.providerId, status: "contract_required", displayName: "쿠콘 / Coocon", coverage: "等待正式 API 文档和响应字段合同", containsCompleteBenefits: false, message: "凭据已设置，但尚未安装签约接口合同，系统不会猜测接口路径" };
    return { providerId: this.providerId, status: "ready", displayName: "쿠콘 / Coocon", coverage: "以当前商业合同为准", containsCompleteBenefits: true };
  }
